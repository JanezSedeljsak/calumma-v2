...
/// so a slow SQLite write can never land inside a render-thread frame budget. The thread holds
/// a `Weak` rather than a borrow: `Engine`'s `Drop` calls `stop` (which joins) before the last
/// `Arc` goes, and if that ever failed to happen the upgrade simply returns `None` and the
/// thread retires itself instead of touching freed memory.
///
...
pub(crate) fn spawn(engine: Weak<Mutex<Inner>>) -> AutosaveThread {
    ...
    let handle = std::thread::spawn(move || {
        ...
        loop {
            ...
            drop(stopped);
            if timed_out {
                let Some(mutex) = engine.upgrade() else {
                    return;
                };
