...
/// The whole point of `repivoted`: forwarding the same local point through the repivoted
/// transform (about the new pivot) must land exactly where the original transform put it
/// (about the old pivot) — a non-uniform scale included, since unlike composing an extra
/// rotation this solves for the offset directly rather than approximating.
#[test]
fn repivoted_keeps_forward_identical_from_a_different_pivot() {
    let old_pivot = (40.0, 20.0);
    let new_pivot = (10.0, 90.0);
    let t = LayerTransform {
        offset_x: 8.0,
        offset_y: -5.0,
        scale_x: 1.4,
    ...
    let repivoted = t.repivoted(old_pivot, new_pivot);
    assert_eq!(repivoted.scale_x, t.scale_x);
...
#[test]
fn repivoting_to_the_same_pivot_is_a_no_op() {
    let t = LayerTransform {
        offset_x: 3.0,
        offset_y: 4.0,
        scale_x: 1.2,
    ...
    let pivot = (12.0, 8.0);
    assert_eq!(t.repivoted(pivot, pivot), t);
}
